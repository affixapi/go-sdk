...
func (v *CurrencyResponse) UnmarshalJSON(src []byte) error {
	...
	enumTypeValue := CurrencyResponse(value)
	for _, existing := range allowedCurrencyResponseEnumValues {
	...
	return fmt.Errorf("%+v is not a valid CurrencyResponse", value)
}

// NewCurrencyResponseFromValue returns a pointer to a valid CurrencyResponse
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewCurrencyResponseFromValue(v string) (*CurrencyResponse, error) {
	ev := CurrencyResponse(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for CurrencyResponse: valid values are %v", v, allowedCurrencyResponseEnumValues)
	}
}
